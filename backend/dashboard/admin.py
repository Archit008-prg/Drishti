...
class ProjectAdmin(admin.ModelAdmin):
    ...
    def approve_reports(self, request, queryset):
        ...
        for report in Report.objects.filter(project__in=queryset):
            report.status = 'approved'
            report.save()
        self.message_user(request, f"Approved reports for {queryset.count()} projects.")
    ...
    def request_resubmission(self, request, queryset):
        ...
        for report in Report.objects.filter(project__in=queryset):
            ...
            report.save()
        self.message_user(request, f"Resubmission requested for {queryset.count()} projects.")
...
class ReportAdmin(admin.ModelAdmin):
    ...
    def approve_selected_reports(self, request, queryset):
        ...
        Project.objects.filter(project_reports__in=queryset).update(
            report_approved=True,
            report_resubmit_requested=False
        )
        self.message_user(request, f'{queryset.count()} reports were successfully approved.')
    ...
    def reject_selected_reports(self, request, queryset):
        ...
        self.message_user(request, f'{queryset.count()} reports were rejected.')
